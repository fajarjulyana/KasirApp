...
def admin_edit_item(item_id):
    ...
    if request.method == 'POST':
        ...
        # Calculate profit
        profit = calculate_total_profit(harga_awal, harga_jual, item['stok_awal'], stok_akhir)
        
        # Update item
        conn = get_db_connection()
        conn.execute('''
            UPDATE barang SET kode = ?, nama = ?, harga_awal = ?, harga_jual = ?, 
                            stok_akhir = ?, profit = ?
            WHERE id = ?
        ''', (kode, nama, harga_awal, harga_jual, stok_akhir, profit, item_id))
        conn.commit()
        conn.close()
